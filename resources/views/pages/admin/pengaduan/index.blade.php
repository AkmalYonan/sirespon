<x-app-layout>
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">Manajemen Pengaduan</h1>
                <p class="text-xs sm:text-sm text-slate-400 mt-1">Daftar seluruh pengaduan masalah dan keluhan yang masuk.</p>
            </div>
        </div>

        <!-- Filter & Search Bar -->
        <div class="glass-card rounded-2xl p-4 border border-slate-800">
            <form action="{{ route('admin.pengaduan.index') }}" method="GET" class="grid sm:grid-cols-4 gap-3">
                <!-- Search Input -->
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.pengaduan.index') }}" class="px-3.5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-semibold flex items-center justify-center transition-smooth">
                        Reset
                    </a>
                    @endif
                </div>
                ...
                <table class="w-full text-left text-xs text-slate-300">
                    <thead class="bg-slate-950/80 text-slate-400 uppercase font-bold text-[10px] tracking-wider border-b border-slate-800">
                        <tr>
                            ...
                            <th scope="col" class="px-5 py-4">Judul Pengaduan</th>
                    ...
                    <tbody class="divide-y divide-slate-800/80">
                        ...
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-slate-400">
                                ...
                                <p class="text-sm font-semibold">Tidak ada data pengaduan ditemukan.</p>
                            </td>
                        </tr>
                        @else
